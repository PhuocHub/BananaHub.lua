...
-- ===== ADMIN TAB (ĐẶT ĐẦU TIÊN) =====
AdminTab = Window:AddTab("Admin")

local KamuiLoaded = false

local KamuiBtn = AdminTab:AddButton({
    ...
    Callback = function()
        if not KamuiLoaded then
            ...
            if KamuiBtn.Set then
                KamuiBtn:Set("Kamui by Phuoc (ON)")
            elseif KamuiBtn.Update then
                KamuiBtn:Update("Kamui by Phuoc (ON)")
            else
                KamuiBtn.Title = "Kamui by Phuoc (ON)"
            end
        end
    end
})
